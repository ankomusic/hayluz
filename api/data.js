...
const { sanitizePrompt, sanitizeJSONResponse, callOpenRouterWithRetry, fetchWithTimeout, setCorsHeaders, apiError, apiSuccess } = require('./utils/helpers');
...
async function handleReport(req, body) {
  ...
  if (url && key) {
    const since = status === 'ok' ? EMPTY_VALUE : new Date().toLocaleTimeString('es-VE', { hour:'2-digit', minute:'2-digit', timeZone: 'America/Caracas' });
    const sanitizedCause = cause ? sanitizePrompt(cause).slice(0, 500) : EMPTY_VALUE;
    const sanitizedNote = reporterNote ? sanitizePrompt(reporterNote).slice(0, 1000) : null;
    
    const ins = await fetchWithTimeout(`${url}/rest/v1/outages?on_conflict=parroquia`, {
      method: 'POST',
...
async function handleGet(query) {
      ...
      if (r.ok) {
        ...
        if (Array.isArray(rows) && rows.length) {
          ...
          sectors = PARROQUIAS.map(name => {
            ...
            if (!row) return { name, status:'nodata', hours:0, since:EMPTY_VALUE, cause:EMPTY_VALUE, affected:0 };
            return {
              name,
              status: STATUSES.includes(row.status) ? row.status : 'ok',
  ...
  if (!sectors) sectors = PARROQUIAS.map(n => ({ name:n, status:'nodata', hours:0, since:EMPTY_VALUE, cause:EMPTY_VALUE, affected:0 }));

  return {
    status: 200,
    headers: { 'Cache-Control': 's-maxage=25, stale-while-revalidate=5' },
    body: { sectors, source, fetchedAt: new Date().toISOString(), city: 'Maracaibo', apiVersion: 'v1' }
  };
}
