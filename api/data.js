/**
 * /api/data  — GET
 * Returns current outage status for all parroquias.
 * Sources: Supabase → Twitter+OpenRouter AI → fallback
 *
...
async function callOpenRouter(system, user, model = 'google/gemini-flash-1.5') {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({ model, max_tokens: 1500, temperature: 0.1,
...
async function fetchFromSupabase() {
  ...
  try {
    ...
    return PARROQUIAS.map(name => {
      ...
      return { name,
        status:   ['ok','inter','cut'].includes(r.status) ? r.status : 'ok',
...
async function fetchTwitter() {
  ...
  try {
    const res = await fetch(`https://api.twitter.com/2/tweets/search/recent?query=${q}&max_results=20&tweet.fields=text`,
