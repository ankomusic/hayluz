...
async function logOutageEvent(
  supabaseUrl,
  serviceKey,
  { parish, previousStatus, newStatus, cause, notes, hoursBeforeEvent = 0 }
) {
  const eventType =
    newStatus === 'cut'
      ? 'started'
      : newStatus === 'inter'
        ...
        : previousStatus === 'cut'
          ? 'restored'
          : previousStatus === 'inter'
            ? 'restored'
            : 'updated';

  try {
    await fetch(`${supabaseUrl}/rest/v1/outage_events`, {
      method: 'POST',
      headers: {
    ...
    console.error('Failed to log outage event:', e.message);
  }
}
...
async function handleAnalyze(body) {
  ...
  if (!prompt) {
    return { status: 400, body: apiError(400, 'Prompt required') };
  }
  ...
  try {
    const result = await callOpenRouterWithRetry(
      sanitizedSystem ||
        'Eres un experto en el sistema eléctrico de Maracaibo, Venezuela. Responde en español.',
      sanitizedPrompt
    );
    ...
    return {
      status: 503,
      body: apiError(503, 'AI service temporarily unavailable', {
        retry: true
      })
    };
  }
...
async function handleVerify(body) {
  const { report } = body;
  if (!report) {
    return { status: 400, body: apiError(400, 'Report required') };
  }
...
{"score":0-100,"verdict":"Verificado|Probable|Dudoso|Falso","confidence":"Alta|Media|Baja","indicators":{"hasLocation":bool,"hasTime":bool,"hasSource":bool,"hasDetails":bool,"consistentWithGrid":bool},"flags":["..."],"summary":"..."}`;
  ...
  try {
    ...
    return {
      status: 503,
      body: apiError(503, 'Verification service unavailable')
    };
  }
}
...
async function handleReport(req, body) {
  ...
  if (!parroquia || !status) {
    return {
      status: 400,
      body: apiError(400, 'parroquia and status required')
    };
  }
  if (!['ok', 'inter', 'cut'].includes(status)) {
    return {
      status: 400,
      body: apiError(400, 'invalid status - must be ok, inter, or cut')
    };
  }
  if (!PARROQUIAS.includes(parroquia)) {
    return {
      ...
      body: apiError(
        400,
        `Invalid parroquia - must be one of: ${PARROQUIAS.join(', ')}`
      )
    };
  }
  ...
  if (process.env.OPENROUTER_API_KEY && reporterNote) {
    try {
      ...
      const t = await callOpenRouterWithRetry(
        ...
        `¿Reporte creíble? Parroquia:${parroquia} Estado:${status} Nota:${sanitizedNote}`,
        80
      );
  ...
  if (confidence === 'low') {
    return {
      status: 422,
      body: apiError(422, 'Reporte no pasó validación', { confidence })
    };
  }
  ...
  if (url && key) {
    ...
    const sanitizedNote = reporterNote
      ? sanitizePrompt(reporterNote).slice(0, 1000)
      : null;
    ...
    if (!ins.ok) {
      return {
        status: 500,
        body: apiError(500, 'DB write failed', { detail: await ins.text() })
      };
    }

    await logOutageEvent(url, key, {
      parish: parroquia,
      previousStatus: null,
      newStatus: status,
      cause: sanitizedCause,
      notes: sanitizedNote,
      hoursBeforeEvent: status === 'ok' ? 0 : 1
    });
  }
  ...
  return {
    ...
    body: apiSuccess({
      ok: true,
      confidence,
      reportsRemaining: MAX_REQUESTS - (count || 1)
    })
  };
}
      ...
      if (r.ok) {
        ...
        if (Array.isArray(rows) && rows.length) {
          ...
          for (const row of rows) {
            if (row.parroquia && !seen.has(row.parroquia)) {
              seen.set(row.parroquia, row);
            }
          }
          sectors = PARROQUIAS.map((name) => {
            ...
            return {
              name,
              status: ['ok', 'inter', 'cut'].includes(row.status)
                ? row.status
                : 'ok',
...
async function handleGetReports(query) {
  ...
  try {
    ...
    return {
      status: 200,
      body: apiSuccess({ reports: Array.isArray(rows) ? rows : [] })
    };
  } catch {
    return { status: 200, body: apiSuccess({ reports: [] }) };
  }
...
async function handleGetEvents(query) {
  const url = process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_ANON_KEY;
  const limit = Math.min(parseInt(query?.limit) || 50, 100);

  if (!url || !key) {
    return { status: 200, body: apiSuccess({ events: [] }) };
  }

  try {
    const r = await fetch(
      `${url}/rest/v1/outage_events?select=*&order=created_at.desc&limit=${limit}`,
      {
        headers: {
          apikey: key,
          Authorization: `Bearer ${key}`,
    ...
    return {
      status: 200,
      body: apiSuccess({ events: Array.isArray(rows) ? rows : [] })
    };
  } catch {
    return { status: 200, body: apiSuccess({ events: [] }) };
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (action === 'analyze') {
        ...
        result = await handleReport(req, body);
      } else if (action === 'reports') {
        result = await handleGetReports(body);
      } else if (action === 'events') {
        result = await handleGetEvents(body);
      } else {
        result = {
          status: 400,
          body: apiError(
            400,
            'action required: analyze|verify|report|reports|events'
          )
        };
      }
      return res.status(result.status).json(result.body);
      ...
      return res
        .status(500)
        .json(apiError(500, 'Internal server error', { detail: e.message }));
    }
  }
